function progresspanel(varargin)
% Description:
%   progresspanel() provides an indication of the progress of some task using
% graphics and text. Calling progresspanel repeatedly will update the figure and
% automatically estimate the amount of time remaining.
%   This implementation of progresspanel is intended to be extremely simple to use
% while providing a high quality user experience.
%
% Features:
%   - Can add progresspanel to existing m-files with a single line of code.
...
%   progresspanel               % Initialize/reset
%   progresspanel(0)            % Initialize/reset
%   progresspanel('Label')      % Initialize/reset and label the bar
%   progresspanel(0.5)          % Update
%   progresspanel(1)            % Close
%
% Example Function Calls For Multi Bar Usage:
%   progresspanel(0, 0)         % Initialize/reset two bars
%   progresspanel('A', '')      % Initialize/reset two bars with one label
%   progresspanel('', 'B')      % Initialize/reset two bars with one label
%   progresspanel('A', 'B')     % Initialize/reset two bars with two labels
%   progresspanel(0.3)          % Update 1st bar
%   progresspanel(0.3, [])      % Update 1st bar
%   progresspanel([], 0.3)      % Update 2nd bar
%   progresspanel(0.7, 0.9)     % Update both bars
%   progresspanel(1)            % Close
%   progresspanel(1, [])        % Close
%   progresspanel(1, 0.4)       % Close
%
% Notes:
%   For best results, call progresspanel with all zero (or all string) inputs
...
%     % Single bar
%     m = 500;
%     progresspanel % Init single bar
%     for i = 1:m
%       pause(0.01) % Do something important
%       progresspanel(i/m) % Update progress bar
%     end
%
...
%     n = 3;
%     p = 100;
%     progresspanel(0,0,0) % Init 3 bars
%     for i = 1:m
%         progresspanel([],0) % Reset 2nd bar
%         for j = 1:n
%             progresspanel([],[],0) % Reset 3rd bar
%             for k = 1:p
%                 pause(0.01) % Do something important
%                 progresspanel([],[],k/p) % Update 3rd bar
%             end
%             progresspanel([],j/n) % Update 2nd bar
%         end
%         progresspanel(i/m) % Update 1st bar
%     end
%
...
%     n = 3;
%     p = 100;
%     progresspanel('Monte Carlo Trials','Simulation','Component') % Init 3 bars
%     for i = 1:m
%         for j = 1:n
%             for k = 1:p
%                 pause(0.01) % Do something important
...
%                 frac2 = ((j-1) + frac3) / n;
%                 frac1 = ((i-1) + frac2) / m;
%                 progresspanel(frac1, frac2, frac3)
%             end
%         end
%     end
%
