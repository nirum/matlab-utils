...
/* const mxArray *pmData, const mxArray *pveY, const mxArray *pveX) */
{
    ...
    */
    ...
    /* memory check */
    if (!((int)vnDimIn)) { mexErrMsgTxt("memory alloc error - vnDimIn"); }
    if (!((int)vnDimOut)) { mexErrMsgTxt("memory alloc error - vnDimOut"); }
    if (!((int)vnEdge)) { mexErrMsgTxt("memory alloc error - vnEdge"); }
    if (!((int)vCoord)) { mexErrMsgTxt("memory alloc error - vCoord"); }
    if (!((int)vpvEdge)) { mexErrMsgTxt("memory alloc error - vpvEdge"); }
    if (!((int)vnAdd)) { mexErrMsgTxt("memory alloc error - vnAdd"); }
